...
void Row::push(unsigned i, const Mat3x3f &val) {
    assert(state != COUNTING);
...
void DynCSRMat::start_rebuild_buffer() {
    ...
    DISPATCH_START(nrow)
    [rows, fixed_row_offsets, fixed_index_buff] (unsigned i) mutable {
...
void DynCSRMat::finish_rebuild_buffer(unsigned &max_nnz_row,
    ...
    DISPATCH_START(nrow)
    [dyn_row_offsets, tmp_array, rows] (unsigned i) mutable {
...
void DynCSRMat::dry_push(unsigned row, unsigned col) {
    if (row <= col) {
        rows[row].dry_push(col);
    }
}

void DynCSRMat::push(unsigned row, unsigned col,
                                const Mat3x3f &val) {
    if (row <= col) {
...
DynCSRMat DynCSRMat::clear() {
    ...
    [rows] (unsigned i) mutable { rows[i].clear(); } DISPATCH_END;
    return *this;
}
...
void DynCSRMat::finalize() {
    ...
    Vec<Row> rows = this->rows;
    DISPATCH_START(rows.size)
    [rows] (unsigned i) mutable { rows[i].finalize(); } DISPATCH_END;
    ...
    DISPATCH_START(nrow)
    [fixed_row_offsets, rows] (unsigned i) mutable {
    ...
    DISPATCH_START(nrow)
    [rows, ref_index_offsets] (unsigned i) mutable {
    ...
    [rows, ref_index_offsets, ref_index_buff,
     ref_value_buff] (unsigned i) mutable {
...
unsigned DynCSRMat::nnz(unsigned row) const {
    return rows[row].head;
}

Mat3x3f DynCSRMat::operator()(unsigned i, unsigned j) const {
...
Mat3x3f FixedCSRMat::operator()(unsigned i, unsigned j) const {
    Mat3x3f val = Mat3x3f::Zero();
    if (!value.data) {
        return val;
    }
...
bool FixedCSRMat::push(unsigned i, unsigned j, const Mat3x3f &val) {
    bool found = false;
    if (i <= j) {
        unsigned nrow = index.offset[index.size];
        unsigned start = index.offset[i];
...
void FixedCSRMat::push_at(unsigned slot, const Mat3x3f &val) {
...
bool FixedCSRMat::exists(unsigned i, unsigned j) const {
    bool found = false;
    if (i <= j) {
        unsigned nrow = index.offset[index.size];
        unsigned start = index.offset[i];
