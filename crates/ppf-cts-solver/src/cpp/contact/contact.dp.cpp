...
unsigned g_ccd_overlap = 0u;
unsigned g_ccd_overlap_v0 = 0xFFFFFFFFu;
unsigned g_ccd_overlap_v1 = 0xFFFFFFFFu;
unsigned g_ccd_overlap_kind = 0xFFFFFFFFu;
float g_ccd_overlap_d2 = -1.0f;
float g_ccd_overlap_offset = -1.0f;
...
inline void record_overlap_pair(unsigned v0, unsigned v1,
                                           unsigned kind) {
...
__forceinline__ float combine_friction(float a, float b, FrictionMode mode) {
    switch (mode) {
...
bool edge_has_shared_vert(const Vec2u &e0, const Vec2u &e1) {
    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 2; j++) {
            if (e0[i] == e1[j])
                return true;
...
void check_success(Vec<char> array) {
    DISPATCH_START(array.size)[array] (unsigned i) {
        assert(array[i] == 0);
    }
    DISPATCH_END;
}
...
template <typename F> struct AABB_AABB_Tester {
    AABB_AABB_Tester(F &op) : op(op) {}
    bool operator()(unsigned index) { return op(index); }
...
inline Vec3f grain_clamp_spin(const Vec3f &spin, const Vec3f &dx,
                                         const Vec3f &n) {
...
inline void accumulate_grain_schur(const Mat3x3f &fric_hess,
...
struct ContactPairCache {
    ...
    unsigned cap{0};
    void record(unsigned a, unsigned b) const {
...
bool statistics_enabled(const DataSet &data) {
    return data.statistics_contact_count.size > 0;
}

void record_object_contact(const DataSet &data, unsigned object_a,
                                      unsigned object_b) {
...
void record_dynamic_contact(const DataSet &data, unsigned vertex_a,
                                       unsigned vertex_b) {
...
void record_dynamic_static_contact(const DataSet &data,
...
void record_analytic_contact(const DataSet &data,
                                        unsigned dynamic_vertex) {
    if (!statistics_enabled(data)) {
        return;
    }
...
unsigned embed_vertex_constraint_force_hessian(
    ...
    if (prop.fix_index > 0) {
        ...
        if (prop.pdrd_body_index > 0 || param.disable_pin_dof_removal) {
            ...
            if (fix.kinematic) {
                gap = sycl::fmax(gap, param.constraint_tol * fix.ghat);
            } else {
                assert(gap >= 0.0f);
            }
        ...
        for (unsigned j = 0; j < data.constraint.floor.size; ++j) {
            ...
            if (e.dot(up) < 0.0f) {
                ...
                if (floor.kinematic) {
                    gap = sycl::fmax(gap, param.constraint_tol * ghat);
                }
                assert(gap >= 0.0f);
...
unsigned embed_contact_force_hessian(const DataSet &data,
    ...
    for (int stage = 0; stage < 2; ++stage) {
        ...
        if (replay) {
            ...
            [data, eval_x, rod_count, contact_force_vec, force, fixed_hess_in,
             ...
             pe_cache, pp_cache] (unsigned i) mutable {
        ...
        if (replay) {
            ...
            [data, eval_x, contact_force_vec, force, fixed_hess_in, fixed_out,
             ...
             stage, dt, edge_active, param, ee_cache] (unsigned i)
                mutable {
    ...
    [force, contact_force_vec] (unsigned i) mutable {
...
unsigned embed_constraint_force_hessian(const DataSet &data,
    ...
    if (!param.disable_contact) {
        ...
        [data, eval_x, force, fixed_hess_in, fixed_out, args, dt,
         ...
         vert_active, param] (unsigned i) mutable {
            Mat3x3f local_hess = fixed_hess_in(i, i);
            const VertexProp &prop = data.prop.vertex[i];
        ...
        [data, eval_x, force, fixed_hess_in, fixed_out, args,
         ...
         collision_vertex_params, dt, param] (unsigned i) mutable {
        ...
        [data, eval_x, force, fixed_hess_in, fixed_out, args,
         ...
         edge_active, param] (unsigned i) mutable {
...
void vertex_constraint_line_search(const DataSet &data,
                                              const Vec<Vec3f> &y0,
                                              const Vec<Vec3f> &y1,
                                              Vec<float> toi_vert,
                                              ParamSet param,
...
float line_search(const DataSet &data, const Vec<Vec3f> &x0,
    ...
    [data, x0, x1, toi_vtf_vec, param,
     pin_infeasible] (unsigned i) mutable {
    ...
    if (!param.disable_contact) {
        ...
        [data, mesh, x0, x1, edge_bvh, edge_aabb, toi_ee_vec, edge_params,
         edge_active, T_vf_seed, param] (unsigned i) mutable {
            // Morton remap: leaf t of the edge BVH stores primitive
            // node[t][0]-1 in Morton order, so adjacent lanes get spatially
            // adjacent queries; their BVH traversals touch overlapping node
            // sets (broadcast/L1 hits instead of scattered loads). Pure
...
// relative to `b0` (v0 = 0).
bool edge_triangle_intersect(const Vec3f &a0, const Vec3f &a1,
...
class CollisionMeshFaceEdgeIntersectTester {
  public:
    CollisionMeshFaceEdgeIntersectTester(const Vec<Vec3f> &vertex,
                                                    const Vec<Vec3u> &face,
                                                    const Vec3f &y0,
                                                    const Vec3f &y1,
                                                    unsigned edge_index,
...
bool check_intersection(const DataSet &data, const Vec<Vec3f> &vertex,
    ...
    if (surface_vert_count > 0) {
        ...
        [data, vertex, vertex_bvh, vertex_aabb, vertex_params, vert_flag_vec,
         records_vec, counter_vec, vert_active] (unsigned i) mutable {
            const VertexParam &vparam =
                vertex_params[data.prop.vertex[i].param_index];
