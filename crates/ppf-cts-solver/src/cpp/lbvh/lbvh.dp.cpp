...
unsigned morton_code_3d(unsigned x, unsigned y, unsigned z) {
    return expand_bits(x) | (expand_bits(y) << 1) | (expand_bits(z) << 2);
}

void compute_face_centroids_kernel(const Vec3f *vertex,
...
void compute_edge_centroids_kernel(const Vec3f *vertex,
...
void compute_vertex_centroids_kernel(const Vec3f *vertex,
                                                unsigned n, float *cx,
                                                float *cy, float *cz) {
    auto item_ct1 = sycl::ext::oneapi::this_work_item::get_nd_item<3>();
    unsigned i = blockIdx.x * blockDim.x + threadIdx.x;
...
void atomicMinFloat(float *addr, float value) {
...
void atomicMaxFloat(float *addr, float value) {
    int *addr_as_int = (int *)addr;
    int old = *addr_as_int;
    int expected;
    do {
...
void compute_scene_bounds_kernel(const float *cx, const float *cy,
...
void init_bounds_kernel(float *bounds) {
    bounds[0] = bounds[1] = bounds[2] = FLT_MAX;
    bounds[3] = bounds[4] = bounds[5] = -FLT_MAX;
}

void compute_morton_codes_kernel(const float *cx, const float *cy,
                                            const float *cz, unsigned n,
                                            const float *bounds,
    ...
    unsigned ix = (unsigned)sycl::fmin(
        1023.0f, sycl::fmax(0.0f, (cx[i] - min_x) * scale_x));
    unsigned iy = (unsigned)sycl::fmin(
        1023.0f, sycl::fmax(0.0f, (cy[i] - min_y) * scale_y));
    unsigned iz = (unsigned)sycl::fmin(
        1023.0f, sycl::fmax(0.0f, (cz[i] - min_z) * scale_z));
...
int clz(unsigned x) { return x == 0 ? 32 : __clz(x); }

// Compute longest common prefix length between Morton codes at indices i and j
int longest_common_prefix(const unsigned *morton_codes, int n, int i,
                                     int j) {
    if (j < 0 || j >= n) {
        return -1;
    }
...
void find_split(const unsigned *morton_codes, int n, int i,
...
void init_parent_kernel(unsigned *parent, unsigned num_nodes) {
    auto item_ct1 = sycl::ext::oneapi::this_work_item::get_nd_item<3>();
    unsigned i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < num_nodes) {
...
void build_internal_nodes_kernel(const unsigned *morton_codes,
...
void set_parent_kernel(const Vec2u *nodes, unsigned *parent,
...
void find_and_set_root_kernel(unsigned *parent, unsigned *root_idx,
...
void compute_depths_kernel(unsigned num_nodes, unsigned root_idx,
...
// Find maximum depth
void find_max_depth_kernel(const unsigned *depth, unsigned n,
                                      unsigned *max_depth) {
    auto item_ct1 = sycl::ext::oneapi::this_work_item::get_nd_item<3>();
...
void count_levels_kernel(const unsigned *depth, unsigned n,
                                    unsigned *level_counts) {
    auto item_ct1 = sycl::ext::oneapi::this_work_item::get_nd_item<3>();
    unsigned i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) {
        dpct::atomic_fetch_add<sycl::access::address_space::generic_space>(
            &level_counts[depth[i]], 1);
    }
}

// Scatter nodes to their level positions
void scatter_to_levels_kernel(const unsigned *depth, unsigned n,
...
compute_leaf_aabbs_face_kernel(const Vec3f *x0, const Vec3f *x1,
    ...
    for (int d = 0; d < 3; ++d) {
        ...
        float minv1 =
            sycl::fmin(float(z10[d]), sycl::fmin(float(z11[d]), float(z12[d])));
        float maxv1 =
            sycl::fmax(float(z10[d]), sycl::fmax(float(z11[d]), float(z12[d])));
        box.min[d] = float(sycl::fmin(minv0, minv1) - margin);
        box.max[d] = float(sycl::fmax(maxv0, maxv1) + margin);
    }
    box.active = true;
    aabb[leaf_idx] = box;
}
...
compute_leaf_aabbs_edge_kernel(const Vec3f *x0, const Vec3f *x1,
    ...
    for (int d = 0; d < 3; ++d) {
        ...
        float minv1 = sycl::fmin(float(z10[d]), float(z11[d]));
        float maxv1 = sycl::fmax(float(z10[d]), float(z11[d]));
        box.min[d] = float(sycl::fmin(minv0, minv1) - margin);
        box.max[d] = float(sycl::fmax(maxv0, maxv1) + margin);
    }
    box.active = true;
...
void compute_leaf_aabbs_vertex_kernel(
    ...
    AABB box;
    for (int d = 0; d < 3; ++d) {
        float minv = sycl::fmin(float(y0[d]), float(z1[d]));
        float maxv = sycl::fmax(float(y0[d]), float(z1[d]));
...
void merge_aabbs_kernel(const unsigned *level_nodes,
...
void merge_level_kernel(const unsigned *level_data,
                                   const unsigned *level_offsets,
