...
float energy(const Vec3f &x0, const Vec3f &x1, float l0) {
    Vec3f t = x1 - x0;
    float r = t.norm() / l0 - 1.0f;
    return 0.5f * r * r;
}

void make_diff_table(const Vec3f &x0, const Vec3f &x1, float l0,
    ...
    float r = (l - l0) / l;
    float c0 = sycl::fmax(0.0f, 1.0f - r) / l0;
    float c1 = sycl::fmax(0.0f, r / l0);
