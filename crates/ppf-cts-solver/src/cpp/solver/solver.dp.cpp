...
inline float cg_curvature_bound(float reduction_scale) {
...
// its coverage.
inline CurvatureVerdict cg_curvature_verdict(float pAp,
                                                                 float bound) {
    if (pAp > bound) {
        return CURVATURE_OK;
    }
...
struct UnrolledMat3x3f {
    const float *data;
    UnrolledMat3x3f(const float *data) : data(data) {}
...
void apply(const DynCSRMat &A, const FixedCSRMat &B, const Vec<Mat3x3f> &C,
    ...
    [A, B, C, D, result, x] (unsigned i) mutable {
  ...
  public:
    ...
    void precond(const Vec<float> &x, Vec<float> &result,
        ...
        if (H && !force_bj) {
            ...
            DISPATCH_QUEUE_START(A.nrow, queue)
            [x, result, inv_diag] (unsigned i) mutable {
...
static Mat3x3f invert(const Mat3x3f &m) {
...
// process.
static dpct::queue_ptr cg_device_stream() {
    static dpct::queue_ptr s = nullptr;
    if (!s) {
...
void cg_fused_update_kernel(const float *p, const float *Ap,
...
void cg_reduce2_kernel(const float *rz_partials,
...
void cg_update_xr_kernel(const float *p, const float *Ap, float *x,
...
void cg_reduce_dot_kernel(const float *dot_partials,
...
void cg_reduce1_kernel(const float *partials, unsigned count,
                                  float *out) {
    auto item_ct1 = sycl::ext::oneapi::this_work_item::get_nd_item<3>();
    __shared__ float s[CGF_BLOCK];
    float acc = 0.0f;
...
void cg_apply_dot_sym_kernel(DynCSRMat A, FixedCSRMat B, Vec<Mat3x3f> C,
...
void cg_apply_dot_walk_kernel(DynCSRMat A, FixedCSRMat B,
...
void cg_beta_kernel(float *d_beta, float *d_rz0, const float *d_rz1,
                               int *breakdown) {
...
cg_device(const DeviceOperators &op, Vec<float> &r, Vec<float> &x,
    ...
    const dpct::queue_ptr q = cg_device_stream();
    ...
    struct GraphGuard {
        /*
        DPCT1119:1: Migration of cudaGraph_t is not supported, please try to
        remigrate with option: --use-experimental-features=graph.
        */
        cudaGraph_t graph{nullptr};
        /*
        DPCT1119:2: Migration of cudaGraphExec_t is not supported, please try to
        remigrate with option: --use-experimental-features=graph.
        */
        ...
        if (try_graph && !graph_ready && iter >= 2) {
            ...
            if (cap == cudaSuccess) {
                ...
                if (cudaStreamIsCapturing(q, &st) == cudaSuccess &&
                    ...
                    /*
                    DPCT1119:3: Migration of cudaGraph_t is not supported,
...
cg_rigid_translation_locked(
    ...
    auto group_l1 = [&](Vec<float> w, float *out) {
        ...
        DISPATCH_START(nb)
        [w, out, body_base] (unsigned b) mutable {
            const float *body = w.data + body_base + 6u * b;
            float sum = 0.0f;
            for (unsigned k = 0; k < 6; ++k) sum += sycl::fabs(body[k]);
            out[1u + b] = sum;
        } DISPATCH_END;
    };
    ...
    auto worst_reresid = [&]() {
        group_l1(r, gc);
        DISPATCH_START(ngrp)
        [gc, g0, gr, err0_all] (unsigned g) mutable {
...
bool solve(const DynCSRMat &A, const FixedCSRMat &B, const Vec<Mat3x3f> &C,
    ...
    [A, B, C, inv_diag_vec] (unsigned i) mutable {
