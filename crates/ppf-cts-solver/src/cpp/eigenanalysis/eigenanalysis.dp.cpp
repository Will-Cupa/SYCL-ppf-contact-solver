...
Mat3x3f expand_U(const Mat3x2f &U) {
...
Mat3x2f compute_force(const DiffTable2 &table, const Svd3x2 &svd) {
...
Mat6x6f compute_hessian(const DiffTable2 &table, const Svd3x2 &svd,
                                   float eps) {
    Vec2f eigvalues;
    Mat2x2f eigvectors;
    ...
    lambda[1] =
        fmaxf(0.0f, sycl::fabs(denom) > eps
...
Mat3x3f compute_force(const DiffTable3 &table, const Svd3x3 &svd) {
...
void accumulate_hessian_tet_fused(const DiffTable3 &table,
...
Mat9x9f compute_hessian(const DiffTable3 &table, const Svd3x3 &svd,
                                   float eps) {
    Vec3f eigvalues;
    Mat3x3f eigvectors;
    utility::solve_symm_eigen3x3(table.d2ed2a, eigvalues, eigvectors);
