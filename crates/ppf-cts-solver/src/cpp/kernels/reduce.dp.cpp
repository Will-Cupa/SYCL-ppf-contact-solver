...
template <typename T, typename Op> T warp_reduce(T val, Op func) {
...
T block_reduce(T val, Op func, T init_val) {
...
__global__ void reduce_op_kernel_2(const Y *input1, const Y *input2, Y *output,
    ...
    auto add_func = [=] (Y a, Y b) { return a + b; };
...
__global__ void inner_product_kernel_optimized<float>(const float *input1, const float *input2, float *output, unsigned n) {
    ...
    const sycl::float2 *vec_input1 =
        reinterpret_cast<const sycl::float2 *>(input1);
    const sycl::float2 *vec_input2 =
        reinterpret_cast<const sycl::float2 *>(input2);
    ...
    for (unsigned i = global_idx; i < vec_n; i += grid_size) {
        ...
        sycl::float2 a0 = vec_input1[base];
        sycl::float2 a1 = vec_input1[base + 1];
        sycl::float2 a2 = vec_input1[base + 2];
        sycl::float2 a3 = vec_input1[base + 3];

        sycl::float2 b0 = vec_input2[base];
        sycl::float2 b1 = vec_input2[base + 1];
        sycl::float2 b2 = vec_input2[base + 2];
        sycl::float2 b3 = vec_input2[base + 3];
    ...
    for (unsigned i = processed + global_idx; i < n; i += grid_size) {
        /*
        DPCT1013:0: The rounding mode could not be specified and the generated
        ...
        */
        thread_sum = sycl::fma(input1[i], input2[i], thread_sum);
    }
...
Y reduce_1(const Y *d_input, Op func, Y init_val, unsigned n) {
    auto dummy = [] (Y a, Y b, Y c) { return a; };
...
template <class T> T sum_array(const T *array, unsigned size) {
    return reduce_1<T>(
        array, [] (T sum, T val) { return sum + val; }, T(), size);
}
...
void sum_into(const float *in, float *out, unsigned n, dpct::queue_ptr queue) {
    if (n == 0) {
        CUDA_HANDLE_ERROR(cudaMemsetAsync(out, 0, sizeof(float), queue));
        return;
    }
