...
void reduce_op_kernel_1(const Y *input, Y *output,
                                   Op func, Y init_val, unsigned n);

template <class Y, typename Op>
void reduce_op_kernel_2(const Y *input1, const Y *input2, Y *output,
                                   Op func, Y init_val, unsigned n);

template <typename Y, typename Op>
void final_reduce_kernel(Y *data, Y *output, Op func, Y init_val, unsigned n);
...
void sum_into(const float *in, float *out, unsigned n,
              dpct::queue_ptr queue = 0);
