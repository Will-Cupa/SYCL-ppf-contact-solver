...
template <typename T>
void set(T *array, unsigned n, T value, dpct::queue_ptr queue) {
    if (n == 0) {
        return;
    } else {
...
template <typename T>
void copy(const T *src, T *dst, unsigned n, dpct::queue_ptr queue) {
    if (n == 0) {
        return;
    } else {
        unsigned block_size = choose_block_size(n);
...
void add_scaled_indirect(const float *src, float *dst, const float *coeff,
                         float sign, unsigned n, dpct::queue_ptr queue) {
    if (n == 0) {
        return;
    }
    unsigned block_size = choose_block_size(n);
...
void scalar_assign(float *dst, const float *src, dpct::queue_ptr queue) {
    scalar_assign_kernel<<<1, 1, 0, queue>>>(dst, src);
    CUDA_HANDLE_ERROR(cudaGetLastError());
}

void flag_if_nonpositive(const float *val, int *flag, dpct::queue_ptr queue) {
