...
Vec3f compute_edge_gradient(const Vec3f &e, float ghat, float offset,
                                       Barrier barrier) {
    float g = e.norm();
    Vec3f n = e / g;
...
Mat3x3f compute_edge_hessian(const Vec3f &e, float ghat,
                                        float offset, Barrier barrier) {
    float c = curvature(e.norm(), ghat, offset, barrier);
    return c * e * e.transpose() / e.squaredNorm();
}

DiffTable2 compute_strainlimiting_diff_table(const Vec2f &a,
                                                        float ghat,
                                                        Barrier barrier) {
    DiffTable2 table;
...
float strainlimiting_energy(const Vec2f &a, float ghat,
                                       Barrier barrier) {
    float result(0.0f);
    for (int i = 0; i < 2; ++i) {
        float g = a[i];
