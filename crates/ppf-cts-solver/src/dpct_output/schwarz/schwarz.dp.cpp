...
inline int find_local(const unsigned *mem, unsigned m, unsigned j) {
...
inline unsigned tri_size(unsigned d) {
    return d * (d + 1u) / 2u;
}
inline unsigned tri_idx(unsigned i, unsigned j) { // requires i >= j
    return i * (i + 1u) / 2u + j;
}

// Transpose a 3x3 block (for the L2/L4 lower-triangle reconstruction when
// materializing M as a both-triangle block-CSR).
inline Mat3x3f transpose3(const Mat3x3f &b) {
    Mat3x3f t;
    for (int r = 0; r < 3; ++r)
        for (int s = 0; s < 3; ++s)
            t(r, s) = b(s, r);
...
inline float frob9(const Mat3x3f &m) {
    float s = 0.0f;
    for (int r = 0; r < 3; ++r)
        for (int c = 0; c < 3; ++c) {
            float v = m(r, c);
...
static inline unsigned grow_cap(size_t n) {
...
static inline size_t factor_shmem(unsigned k) {
...
static bool ensure_dyn_shmem(const void *fn, size_t need, int &cache) {
    ...
    dpct::err0 err = cudaFuncSetAttribute(
...
inline bool try_attach(Vec<unsigned> agg, Vec<unsigned> cnt,
...
void factor_kernel(DynCSRMat A, FixedCSRMat B, Vec<Mat3x3f> C,
...
void apply_kernel(Vec<unsigned> agg_off, Vec<unsigned> members,
                             Vec<unsigned> ainv_off, Vec<float> ainv,
...
void build_fine_graph(const DynCSRMat &A, const FixedCSRMat &B) {
    ...
    Vec<unsigned> fine_off = K.fine_off;
    {
        ...
        FixedCSRMat Bc = B;
        DISPATCH_START(nrow)
        [Ac, Bc, fine_off] (unsigned i) mutable {
    ...
    Vec<float> wgt = K.fine_wgt;
    {
        ...
        FixedCSRMat Bc = B;
        DISPATCH_START(nrow)
        [Ac, Bc, off, col, wgt] (unsigned i) mutable {
...
void agg_core(const unsigned *offset, const unsigned *col, unsigned nrow,
    ...
    if (must_partner) {
        ...
        DISPATCH_START(nrow)
        [aggv, cntv, ctr, mp] (unsigned i) mutable {
            unsigned j = mp[i];
            if (j == UNSET || j <= i) {
                return;
            }
    ...
    for (unsigned round = 0; round < rounds; ++round) {
        if (degseed) { // unset-degree per UNSET node (closed under the round's set)
            Vec<unsigned> aggv = agg;
            DISPATCH_START(nrow)
            [offset, col, aggv, wv] (unsigned i) mutable {
        ...
        { // Seed: a node roots when it is the local maximum among UNSET neighbors,
          // by (unset-degree, mix64(id)) when degseed else (-id) [id local min].
            ...
            DISPATCH_START(nrow)
            [offset, col, aggv, cntv, ctr, wv, ds] (unsigned i) mutable {
                if (aggv.data[i] != UNSET) {
                    return;
                }
                bool is_root = true;
        ...
        { // Attach: each UNSET node joins the first neighbor aggregate with room
          ...
          // FIRST so the stiff coupling is captured intra-aggregate.
            ...
            const unsigned *mp = must_partner;
            DISPATCH_START(nrow)
            [offset, col, aggv, cntv, ks, mp] (unsigned i) mutable {
                if (aggv.data[i] != UNSET) {
                    return;
                }
    ...
    for (unsigned a = 0; a < absorb; ++a) {
        ...
        DISPATCH_START(nrow)
        [offset, col, aggv, cntv, km] (unsigned i) mutable {
            if (aggv.data[i] != UNSET) {
                return;
            }
            for (unsigned k = offset[i]; k < offset[i + 1]; ++k) {
    ...
    { // Any node still unassigned (whole neighborhood full) becomes a singleton.
        ...
        DISPATCH_START(nrow)
        [aggv, ctr, fc] (unsigned i) mutable {
            if (aggv.data[i] == UNSET) {
...
void compute_must_partner(const DynCSRMat &A, const FixedCSRMat &B,
    ...
    DISPATCH_START(nrow)
    [Ac, Bc, Cc, mp] (unsigned i) mutable {
...
void recompute_aggregation(const DynCSRMat &A, const FixedCSRMat &B,
    ...
    [agg_off] (unsigned g) mutable { agg_off.data[g] = 0; } DISPATCH_END;
    {
        Vec<unsigned> aggv = K.agg;
        Vec<unsigned> off = K.agg_off;
        DISPATCH_START(nrow)
        [aggv, off] (unsigned i) mutable {
    ...
    DISPATCH_START(n_agg)
    [cursor] (unsigned g) mutable { cursor.data[g] = 0; } DISPATCH_END;
    {
        Vec<unsigned> aggv = K.agg;
        Vec<unsigned> off = K.agg_off;
        ...
        DISPATCH_START(nrow)
        [aggv, off, members, cur] (unsigned i) mutable {
    ...
    Vec<unsigned> ainv_off = K.ainv_off;
    {
        Vec<unsigned> off = K.agg_off;
        DISPATCH_START(n_agg)
        [ainv_off, off] (unsigned g) mutable {
    ...
    if (getenv("PPF_SCHWARZ_DIAG")) {
        ...
        const unsigned km = kmax;
        DISPATCH_START(n_agg)
        [off, hist, km] (unsigned g) mutable {
...
void factor_kernel_coarse(CoarseMat A, Vec<unsigned> agg_off,
                                     Vec<unsigned> members,
                                     Vec<unsigned> ainv_off, Vec<float> ainv,
                                     unsigned max_dim) {
    auto item_ct1 = sycl::ext::oneapi::this_work_item::get_nd_item<3>();
...
void materialize_level0(const DynCSRMat &A, const FixedCSRMat &B,
    ...
    Vec<Mat3x3f> Cc = C;
    {
        ...
        FixedCSRMat Bc = B;
        DISPATCH_START(nrow)
        [Ac, Bc, Cc, off, col, val] (unsigned i) mutable {
            unsigned w = off.data[i];
            const Row &row = Ac.rows.data[i];
            for (unsigned k = 0; k < row.head; ++k) {
                col.data[w] = row.index[k];
...
void galerkin(const CoarseMat &src, const unsigned *agg, unsigned n_agg,
...

    {
        ...
        [off, col, keyv, permv, agg, na] (unsigned i) mutable {
            unsigned g = agg[i];
    ...
    DISPATCH_START(n_agg)
    [doff] (unsigned g) mutable { doff.data[g] = 0; } DISPATCH_END;
    {
        const unsigned na = n_agg;
        DISPATCH_START(nnz)
        [keyv, eid, dcol, doff, es, na] (unsigned s) mutable {
    ...
    Vec<Mat3x3f> dvalue = dst.value;
    {
        ...
        DISPATCH_START(nnz1)
        [es, permd, sval, dvalue] (unsigned e) mutable {
            float acc[9];
            for (int t = 0; t < 9; ++t) {
                acc[t] = 0.0f;
            }
...
unsigned build_domains(Vec<unsigned> agg, unsigned n, unsigned n_agg,
    ...
    CUDA_HANDLE_ERROR(cudaMemcpy(agg_off.data + n_agg, &total, sizeof(unsigned),
                                 cudaMemcpyHostToDevice));
    DISPATCH_START(n_agg)
    [scratch] (unsigned g) mutable { scratch.data[g] = 0; } DISPATCH_END;
    {
        Vec<unsigned> aggv = agg, off = agg_off, mem = members, cur = scratch;
        DISPATCH_START(n)
        [aggv, off, mem, cur] (unsigned i) mutable {
            unsigned g = aggv.data[i];
            unsigned pos = atomicAdd(&cur.data[g], 1u);
...
void restrict_direct(const Vec<unsigned> &map_fine, const Vec<float> &x,
    ...
    [rcv] (unsigned i) mutable { rcv.data[i] = 0.0f; } DISPATCH_QUEUE_END;
    Vec<unsigned> mp = map_fine;
    Vec<float> xv = x;
    DISPATCH_QUEUE_START(nrow, stream)
    [mp, xv, rcv] (unsigned i) mutable {
...
void prolong_add_direct(const Vec<unsigned> &map_fine, const Vec<float> &ec,
    ...
    DISPATCH_QUEUE_START(nrow, stream)
    [mp, ecv, zv, w] (unsigned i) mutable {
        unsigned g = mp.data[i];
