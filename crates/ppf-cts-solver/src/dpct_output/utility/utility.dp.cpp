...
Vec3f compute_vertex_normal(const DataSet &data,
...
void solve_symm_eigen2x2(const Mat2x2f &matrix, Vec2f &eigenvalues,
                                    Mat2x2f &eigenvectors) {
    linalg::eig::symm2x2(matrix, eigenvalues, eigenvectors);
}

void solve_symm_eigen3x3(const Mat3x3f &matrix, Vec3f &eigenvalues,
...
void jacobi_svd(const SMat<float, 3, N> &F, SMat<float, 3, N> &U,
            ...
            for (int q = p + 1; q < N; ++q) {
                ...
                float t =
                    (g == 0.0f)
                        ? 0.0f
                        : (zeta >= 0.0f ? 1.0f : -1.0f) /
                              (sycl::fabs(zeta) + sqrtf(1.0f + zeta * zeta));
...
Vec2f singular_vals_minus_one(const Mat3x2f &F) {
    Mat3x2f U;
    Vec2f sigma;
    Mat2x2f V;
...
Svd3x2 svd3x2_shifted(const Mat3x2f &F) {
    Mat3x2f U;
    Vec2f sigma;
    Mat2x2f V;
    jacobi_svd<2, 1>(F, U, sigma, V);
...
Svd3x3 svd3x3_rv(const Mat3x3f &F) {
    Svd3x3 svd = svd3x3(F);
...
Mat3x3f convert_force(const Mat3x2f &dedF,
...
Mat3x4f convert_force(const Mat3x3f &dedF,
...
Mat9x9f convert_hessian(const Mat6x6f &d2ed2f,
                                   const Mat2x2f &inv_rest2x2) {
    Vec2f g[3];
...
Mat12x12f convert_hessian(const Mat9x9f &d2ed2f,
                                     const Mat3x3f &inv_rest3x3) {
    Vec3f g0 = -inv_rest3x3.row(0) - inv_rest3x3.row(1) - inv_rest3x3.row(2);
    Vec3f g1 = inv_rest3x3.row(0);
    Vec3f g2 = inv_rest3x3.row(1);
...
Mat3x2f compute_deformation_grad(const Mat3x3f &x,
                                            const Mat2x2f &inv_rest2x2) {
    Mat3x2f dx;
...
Mat3x3f compute_deformation_grad(const Mat3x4f &x,
                                            const Mat3x3f &inv_rest3x3) {
    Mat3x3f dx;
    dx.col(0) = x.col(1) - x.col(0);
    dx.col(1) = x.col(2) - x.col(0);
...
float compute_face_area(const Mat3x3f &vertex) {
...
void compute_svd(DataSet data, Vec<Vec3f> curr, Vec<Svd3x2> svd,
    ...
    [mesh_face, curr_data, svd_data, inv_rest2x2] (unsigned i) mutable {
...
float get_wind_weight(float time) {
