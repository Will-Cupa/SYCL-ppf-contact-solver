...
void eigen_sym3x3(
    ...
    float p = sqrtf(sycl::fmax(p2 / 6.0f, 0.0f));
    ...
    if (p < 1e-12f) {
        ...
        float det_b = b00 * (b11 * b22 - b12 * b12)
                    - b01 * (b01 * b22 - b12 * b02)
                    + b02 * (b01 * b12 - b11 * b02);
        float r = sycl::fmin(1.0f, sycl::fmax(-1.0f, det_b * 0.5f));
...
void embed_vertex_force_hessian(
...
void embed_rod_force_hessian(const DataSet &data,
...
void embed_face_force_hessian(const DataSet &data,
                                         const Vec<Vec3f> &eval_x,
                                         Vec<float> &force, FixedCSRMat &hess,
                                         float dt, const ParamSet &param,
                                         unsigned i) {
...
void embed_tet_force_hessian(const DataSet &data,
                                        const Vec<Vec3f> &eval_x,
                                        Vec<float> &force, FixedCSRMat &hess,
                                        float dt, const ParamSet &param,
                                        unsigned i) {
...
void embed_hinge_force_hessian(const DataSet &data,
                                          const Vec<Vec3f> &eval_x,
                                          Vec<float> &force, FixedCSRMat &hess,
                                          float dt, const ParamSet &param,
                                          unsigned i) {
...
void compute_torque_groups(const DataSet &data,
    ...
    [data, eval_x, result] (unsigned g) mutable {
...
void embed_momentum_force_hessian(const DataSet &data,
    ...
    [data, eval_x, velocity, dt, target, force, diag_hess,
     param, torque_result] (unsigned i) mutable {
...
void embed_stitch_force_hessian(const DataSet &data, const Vec<Vec3f> &eval_x,
    ...
    if (seam_count) {
        ...
        [data, eval_x, force, fixed_out, param] (unsigned i) mutable {
