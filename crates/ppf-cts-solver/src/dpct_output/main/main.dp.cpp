...
void invalidate_inactive_aabbs() {
    ...
    if (fc > 0) {
        DISPATCH_START(fc)
        [fa_ptr, d_fa, face_nodes] (unsigned i) mutable {
    ...
    if (ec > 0) {
        DISPATCH_START(ec)
        [ea_ptr, d_ea, edge_nodes] (unsigned i) mutable {
    ...
    if (svc > 0) {
        DISPATCH_START(svc)
        [va_ptr, d_va, vert_nodes] (unsigned i) mutable {
...
StepResult advance() {
    ...
    if (shell_face_count || rod_count) {
        ...
        if (shell_face_count) {
            ...
            [prop_face, param_face, mesh_face, prop_vertex, svd_vec,
             tmp_scalar_vec] (unsigned i) mutable {
                ...
                if (!prop.fixed) {
                    ...
                    tmp_scalar_vec[i] =
                        fmaxf(svd_vec[i].S[0], svd_vec[i].S[1]) *
                        sycl::fmin(fparam.shrink_x, fparam.shrink_y);
                }
            } DISPATCH_END;
            max_sigma = fmaxf(
                max_sigma,
    ...
    auto compute_target = [&](float dx) {
        ...
        [prop_vertex, constraint_fix, vertex_curr, vertex_prev, target_vec, dx,
         dt, prev_dt, gravity, inactive_momentum] (unsigned i) mutable {
    ...
    auto rewind_kinematic_fix = [&](double toi) {
        ...
        [fix_vec, back] (unsigned i) mutable {
    ...
    if (!disable_dof_removal) {
        ...
        DISPATCH_START(vertex_count)
        [prop_vertex, mask] (unsigned i) mutable {
    ...
    while (true) {
        ...
        if (n_dof_removed > 0) {
            ...
            [dof_mask, elim_rows, elim_fx_off, elim_fx_col, elim_fx_val,
             ...
             elim_target] (unsigned i) mutable {
        ...
        float max_dx;
        {
            ...
            Vec<float> tmp_scalar_vec = tmp_scalar.as_vec();
            DISPATCH_START(vertex_count)
            [dx_vec, tmp_scalar_vec] (unsigned i) mutable {
        ...
        float toi_recale = sycl::fmin(1.0f, prm.max_dx / max_dx);
        ...
        kernels::copy(eval_x.data, target.data, target.size);
        {
            ...
            Vec<float> dx_vec = dx.as_vec();
            DISPATCH_START(vertex_count)
            [eval_x_vec, toi_recale, dx_vec] (unsigned i) mutable {
        ...
        if (rod_count > 0) {
            ...
            SL_toi = sycl::fmin(SL_toi, SL_rod_toi);
            toi = sycl::fmin(toi, SL_rod_toi);
...

        {
            ...
            Vec<Vec3f> target_vec = target.as_vec();
            DISPATCH_START(vertex_count)
            [eval_x_vec, target_vec, toi] (unsigned i) mutable {
        ...
        if (rigid_pdrd) {
            ...
            {
                DISPATCH_START(vertex_count)
                [eval_x_vec, rigid_tgt_vec, toi_rig] (unsigned i) mutable {
                    Vec3f d = toi_rig *
...
extern "C" DLL_EXPORT void override_velocity(
    ...
    [d_indices, dev_curr, dev_prev, fp_vx, fp_vy, fp_vz] (unsigned i) mutable {
...
extern "C" DLL_EXPORT void gather_current_positions(
    ...
    DISPATCH_START(count)
    [d_indices, d_out, dev_curr] (unsigned i) mutable {
        unsigned vi = d_indices[i];
...
extern "C" DLL_EXPORT void refresh_collision_active(float time) {
    ...
    DISPATCH_START(vc)
    [d_va, d_dm, d_w, d_wc, time] (unsigned i) mutable {
        unsigned dm = d_dm[i];
    ...
    DISPATCH_START(fc)
    [d_fa, d_va, faces] (unsigned i) mutable {
        Vec3u f = faces[i];
    ...
    DISPATCH_START(ec)
    [d_ea, d_va, edges] (unsigned i) mutable {
        Vec2u e = edges[i];
